...
/**
 * Get the type of a tuple with @a len elements from position @pos of the original tuple type.
 */
...
struct tuple_type_subset
{
  ...
  using type = typename tuple_type_start<typename tuple_type_end<T, std::tuple_size<T>::value - pos>::type, len>::type;
...
/**
 * Get the a tuple with @a len elements from position @pos of the original tuple type.
 */
template<std::size_t pos, std::size_t len, typename T>
decltype(auto)
tuple_subset(const T& t) {
  return tuple_start<len>(
    tuple_end<std::tuple_size<T>::value - pos>(t)
  );
}
