...
struct tuple_transform_each_const_impl {
  ...
  tuple_transform_each_const(const T& t) {
    ...
    const auto element =
      T_transformer<element_type>::transform(std::get<index>(t));
    ...
    return tuple_transform_each_const_impl<T_transformer,
      index - 1>::tuple_transform_each_const(t_with_transformed_element);
  }
};
...
tuple_transform_each_const(const T& t) {
  constexpr auto size = std::tuple_size<T>::value;
  return tuple_transform_each_const_impl<T_transformer,
    size - 1>::tuple_transform_each_const(t);
}
